...
function StatusDot({ status }: { status: Project["status"] }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
      <span className={`size-2 rounded-full ${config.color} ${status === "live" ? "animate-pulse" : ""}`} />
      {config.label}
    </span>
  );
}
...
function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <Card
      className={`group/project relative transition-all duration-300 hover:ring-2 hover:ring-foreground/20 hover:-translate-y-0.5 ${
        project.size === "lg"
...
export function BentoGrid() {
  ...
  const counts: Record<string, number> = {
    ...
    ...Object.fromEntries(
      categories
        .filter((c) => c.key !== "all")
        .map((c) => [c.key, projects.filter((p) => p.category === c.key).length])
    ),
  };
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {categories.map((cat) => (
          ...
          >
            {cat.label}
            <span className="ml-1.5 text-xs opacity-60">{counts[cat.key]}</span>
          </button>
        ))}
      </div>
